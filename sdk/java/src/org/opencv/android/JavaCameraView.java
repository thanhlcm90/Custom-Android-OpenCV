	...
	protected boolean initializeCamera(int width, int height) {
		...
		synchronized (this) {
			...
			try {
				...
				List<android.hardware.Camera.Size> captureSizes = params
				...
				if (sizes != null) {
					...
					Size pictureSize = calculateCameraFrameSize(captureSizes,
							new JavaCameraSizeAccessor(), width, height);

					params.setPreviewSize((int) frameSize.width,
							(int) frameSize.height);
					Log.d(TAG, "Picture size: " + pictureSize.width + "x"
							+ pictureSize.height);
					params.setPictureSize((int) pictureSize.width,
					...
					if ((getLayoutParams().width == LayoutParams.MATCH_PARENT)
						...
						if (mPortrait) {
							mScale = Math.min(((float) height) / mFrameWidth,
									((float) width) / mFrameHeight);
						} else {
